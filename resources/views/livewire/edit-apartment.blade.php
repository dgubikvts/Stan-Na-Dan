<div>
    ...
    <form class="w-1/2 flex flex-col bg-white m-auto shadow-xl rounded mt-2 p-5" method="post" enctype="multipart/form-data" wire:submit.prevent="saveChanges">
        <h2 class="text-3xl text-center mb-5">Izmeni stan</h2>
        <div class="flex justify-around mb-8 ">
            <div class="flex flex-col w-5/6">
                <label for="title" class="mb-3">Naziv:</label>
                <input type="text" name="title" value="{{$apartment->title}}" wire:model="form.title" class="rounded p-2 border hover:shadow focus:shadow-lg focus:outline-none">
        ...
        <div class="grid grid-cols-3 gap-4 w-5/6 self-center mt-3">
            ...
            <div wire:key="{{$loop->index}}" class="flex flex-col p-4 border border-black rounded">
                <img src="{{ gettype($image) === 'string' ? asset('storage/images/'.$apartment->id.'/'. $image) : $image->temporaryUrl()}}" class="mb-3">
        ...
        @endif
        @if(!$images)
        <button type="submit" class="bg-blue-600 text-white text-xl p-2 rounded w-5/6 self-center mt-8" wire:loading.attr="disabled" wire:loading.class="bg-gray-600" wire:target="images">Sacuvaj promene</button>
        @else
        <button type="submit" class="bg-blue-600 text-white text-xl p-2 rounded w-5/6 self-center mt-8" wire:loading.attr="disabled" wire:loading.class="bg-gray-600" wire:target="moreImages">Sacuvaj promene</button>
        @endif
    </form> 
</div>